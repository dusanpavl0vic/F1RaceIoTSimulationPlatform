...
const resolveStatusChipProps = (statusLabel: string) => {
  switch (statusLabel) {
    case "RET": return { label: "RET", color: "error" as const, variant: "filled" as const };
    case "STOP": return { label: "STOP", color: "error" as const, variant: "outlined" as const };
    case "PIT": return { label: "PIT", color: "warning" as const, variant: "filled" as const };
    case "OUT": return { label: "OUT", color: "warning" as const, variant: "outlined" as const };
    case "RUN": return { label: "RUN", color: "success" as const, variant: "outlined" as const };
    default: return { label: statusLabel === "-" ? "—" : statusLabel, color: "default" as const, variant: "outlined" as const };
  }
};

export function LeaderboardDriverRow({ row, isMobile, isTablet, positionChange }: LeaderboardDriverRowProps) {
  ...
  return (
    <StyledDriverTableRow hover $dimmed={isDimmed} $flash={flash}>
      <StyledPosCell $isLeader={pos === 1}>
        {pos ?? "—"}
        {flash && <StyledPosArrow $direction={flash}>{flash === "gained" ? "▲" : "▼"}</StyledPosArrow>}
      </StyledPosCell>

      <StyledDriverCell $color={teamColor}>
        <StyledDriverTla $color={teamColor}>{row.tla ?? row.driverLabel}</StyledDriverTla>
      ...
      {!isTablet && <StyledMutedCell>{row.intervalToPositionAhead ?? "—"}</StyledMutedCell>}
      ...
      <StyledLastCell>
        <Stack direction="row" spacing={0.5}>
          {row.pitFlag !== "-" && (
            <Chip size="small" label={row.pitFlag} color="warning" variant="filled" />
          )}
